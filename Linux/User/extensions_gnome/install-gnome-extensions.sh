...
args_count="$#"
dependencies=(wget curl tput sed egrep sed awk gnome-shell cut basename)
deps_install_apt="sudo apt install -y wget curl sed"
deps_install_dnf="sudo dnf install -y wget curl sed"
...
function install_shell_extensions() {
    ...
    for ext_id in "${EXTENSIONS_TO_INSTALL[@]}"; do
        ...
        extension_name=$(echo "$ext_info" | grep -Eo '"name": *"[^"]+"' | sed -E 's/"name": *"(.*)"/\1/')
        direct_dload_url=$(echo "$ext_info" | grep -Eo '"download_url": *"[^"]+"' | sed -E 's/"download_url": *"(.*)"/\1/')
        ext_uuid=$(echo "$ext_info" | grep -Eo '"uuid": *"[^"]+"' | sed -E 's/"uuid": *"(.*)"/\1/')
        ext_version=$(echo "$ext_info" | grep -Eo '"version": *[0-9]+' | sed -E 's/"version": *([0-9]+)/\1/')
        ext_homepage=$(echo "$ext_info" | grep -Eo '"url": *"[^"]+"' | sed -E 's/"url": *"(.*)"/\1/')
        ext_description=$(echo "$ext_info" | grep -Eo '"description": *"[^"]+"' | sed -E 's/"description": *"(.*)"/\1/')
