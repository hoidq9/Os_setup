...
mkdir -p /home/$user_current/Bootloader
...
create_one_time_file_enc_sha256() {
	mkdir -p /home/$user_current/Bootloader/one_time_file_enc_sha256
	cd /home/$user_current/Bootloader/one_time_file_enc_sha256 || return
...
grub2_bootloader_setup() {
	...
	if [ "$isLuksBoot" == true ]; then
		...
		build_grubx64_image() {
			...
			./bootstrap
			./autogen.sh
			./configure --prefix="/home/$user_current/Bootloader/Grub2" --with-platform=efi --target=x86_64 --enable-stack-protector --enable-mm-debug --enable-cache-stats --enable-boot-time --enable-grub-emu-sdl2 --enable-grub-emu-sdl --enable-grub-emu-pci --enable-grub-mkfont --enable-grub-mount --enable-device-mapper --enable-liblzma --enable-grub-protect --with-gnu-ld --with-unifont=/usr/share/fonts/unifont/unifont.otf --with-dejavufont=/usr/share/fonts/dejavu-sans-fonts/DejaVuSans.ttf --enable-threads=posix+isoc --enable-cross-guesses=conservative --enable-dependency-tracking # --enable-libzfs --enable-grub-themes
			make install

			cp "$REPO_DIR"/config_grub.cfg /home/$user_current/Bootloader/Grub2/bin/config_"$os_id".cfg
			cp "$REPO_DIR"/sbat_grub.csv /home/$user_current/Bootloader/Grub2/bin/sbat_"$os_id".csv

			grub_version_build=$(grep "grub_version" /home/$user_current/Bootloader/Grub2/lib/grub/x86_64-efi/modinfo.sh | cut -d'"' -f2)
			sed -i "s/(version)/$grub_version_build/g" /home/$user_current/Bootloader/Grub2/bin/sbat_"$os_id".csv
			sed -i "s/(efi_uuid)/$uuid_efi/g" /home/$user_current/Bootloader/Grub2/bin/config_"$os_id".cfg
			...
			sed -i "s/(boot_locked_uuid)/$uuid_boot_luks_locked/g" /home/$user_current/Bootloader/Grub2/bin/config_"$os_id".cfg
			sed -i "s/(boot_unlocked_uuid)/$uuid_boot_luks_unlocked/g" /home/$user_current/Bootloader/Grub2/bin/config_"$os_id".cfg

			cd /home/$user_current/Bootloader/Grub2/bin || return
			...
			if [ "$os_id" == "fedora" ]; then
				...
				pesign --in grubx64_new.efi --out grubx64.efi --certificate "${os_id}-auth" --sign --force
...
create_luks2_boot_partition() {
	...
	if [[ "$boot_dev" != "$root_dev" ]] && [ "$isLuksBoot" == false ] && [ "$isLuksRoot" == true ]; then
		...
		# cp /home/$user_current/Bootloader/one_time_file_enc_sha256/expected.sha256 /boot/expected.sha256
		# cp /home/$user_current/Bootloader/one_time_file_enc_sha256/one_time_random.bin.enc /boot/one_time_random.bin.enc
...
chown -R $user_current:$user_current /home/$user_current/Bootloader
# main |& tee /result.txt
