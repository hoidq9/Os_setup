...
grub2_bootloader_setup() {
	...
	if [ "$isLuksBoot" == true ]; then
		...
		build_grubx64_image() {
			...
			sed -i "s/(boot_locked_uuid)/$uuid_boot_luks_locked/g" /repos/Grub2/bin/config_"$os_id".cfg
			sed -i "s/(boot_unlocked_uuid)/$uuid_boot_luks_unlocked/g" /repos/Grub2/bin/config_"$os_id".cfg
