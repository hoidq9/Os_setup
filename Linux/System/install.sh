...
create_keys_secureboot() {
	...
	if [ ! -f /boot/"${os_id}-auth".der ]; then
		cp /keys/secureboot/"${os_id}-auth".der /boot/efi
	fi
}
...
rhel_system() {
	...
	packages() {
		...
		dnf install zsh PackageKit-command-not-found git dbus-x11 gdb gcc flatpak ibus-m17n podman msr-tools cockpit-machines cockpit-podman cockpit code google-chrome-stable rpcbind portmap xorg-x11-server-Xwayland sshfs rhc rhc-worker-playbook ansible-core yara pcsc-lite xwayland-run mesa-vulkan-drivers baobab -y # dconf-editor gnome-extensions-app.x86_64 podman-compose conky virt-manager redhat-mono-fonts yandex-browser-stable microsoft-edge-stable kernel-devel gnome-shell-extension-argos xisxwayland xorg-x11-server-Xwayland-devel xwaylandvideobridge
